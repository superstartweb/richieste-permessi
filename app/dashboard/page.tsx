...
export default function DashboardPage() {
  ...
  async function initDashboard() {
    ...
    if (email) {
      const { data: profile, error } = await supabase.from('todde_profiles').select('*').eq('email', email.toLowerCase().trim()).maybeSingle()
      if (profile) {
  ...
  async function fetchAllData(isAdmin: boolean, userId: string) {
    ...
    const { data: rData, error } = await rQuery.order('data_inizio', { ascending: false })
    if (error) {
      console.error("Errore caricamento richieste:", error.message)
  ...
  const handleAddEmployee = async (e: React.FormEvent) => {
    ...
    if (error) alert("ERRORE: " + error.message)
      ...
      const msg = `Ciao ${newEmp.nome}, benvenuto in Todde Bus. Accedi all'App con la tua email aziendale da qui: https://permessi.toddebus.it`
  ...
  const handleInvia = async (e: React.FormEvent) => {
    ...
    else result = await supabase.from('todde_richieste').insert([payload])
            ...
            <div className="bg-white p-8 rounded-[45px] shadow-sm">
              ...
              <div className="grid grid-cols-7 gap-3 text-center font-bold">
                ...
                {[...Array(getDaysInMonth(currentDate.getFullYear(), currentDate.getMonth()))].map((_,i)=>{
                  ...
                  return (
                    <div key={d} onClick={()=>setSelectedDate(ds)} className={`h-16 border-2 rounded-2xl flex flex-col items-center justify-center cursor-pointer transition ${selectedDate === ds ? 'border-blue-600 bg-blue-50 text-blue-700' : assenti.length > 0 ? 'bg-orange-50 border-orange-200' : 'bg-white border-transparent'}`}>
            ...
            {selectedDate && (
              <div className="bg-white p-8 rounded-[40px] shadow-lg border-2 border-blue-100 animate-in fade-in slide-in-from-bottom-2">
                ...
                <div className="space-y-2">
                   ...
                   ) : <p className="text-center text-[10px] font-bold text-slate-400 py-4">Nessuna assenza programmata.</p>}
                </div>
              </div>
            )}
          </div>
        ...
        {activeTab === 'nuova' && (
          <div className="max-w-2xl mx-auto bg-white p-10 rounded-[50px] shadow-2xl border-4 border-white font-sans uppercase">
            ...
            <form onSubmit={handleInvia} className="space-y-8 text-center">
              ...
              <button type="submit" className="w-full bg-blue-700 text-white p-6 rounded-[35px] font-black text-xl shadow-xl uppercase tracking-widest active:scale-95 transition-all">
                Conferma
              </button>
            </form>
