...
// Helper fuori dal componente per non dare errori di riferimento
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-10 font-sans">
      ...
      <main className="max-w-5xl mx-auto p-4">
        ...
        {isAdmin && activeTab === 'anagrafica' && (
          <div className="space-y-4">
            <button onClick={() => setShowAddEmployee(!showAddEmployee)} className="w-full bg-blue-700 text-white p-5 rounded-3xl font-bold shadow-lg uppercase">+ AGGIUNGI DIPENDENTE</button>
            <div className="grid gap-3">
              {profiles.map(p => (
                <div key={p.id} className="bg-white p-5 rounded-[30px] flex justify-between items-center shadow-sm border border-slate-100 uppercase">
                  <div className="font-bold text-sm">{p.cognome} {p.nome} <span className="block text-[9px] text-slate-400 normal-case">{p.email}</span></div>
        ...
        {isAdmin && activeTab === 'calendario' && (
          <div className="space-y-6">
            ...
            {selectedDate && (
              <div className="bg-white p-8 rounded-[40px] shadow-lg border-2 border-blue-100">
        ...
        {activeTab === 'nuova' && (
          <div className="max-w-2xl mx-auto bg-white p-10 rounded-[50px] shadow-2xl border-4 border-white font-sans uppercase">
            <h2 className="text-2xl font-black mb-10 text-center tracking-tighter decoration-blue-100">{editId ? 'Aggiorna Richiesta' : 'Invia Comunicazione'}</h2>
            <form onSubmit={handleInvia} className="space-y-8 text-center">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Inizio</label><input type="date" className="w-full p-5 bg-slate-100 rounded-[30px] font-black border-none text-center outline-none" value={form.data_inizio} onChange={e => setForm({...form, data_inizio: e.target.value})} required /></div>
                <div><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Fine (Opz.)</label><input type="date" className="w-full p-5 bg-slate-100 rounded-[30px] font-black border-none text-center outline-none" value={form.data_fine} onChange={e => setForm({...form, data_fine: e.target.value})} /></div>
              </div>
              <button type="submit" className="w-full bg-blue-700 text-white p-6 rounded-[35px] font-black text-xl shadow-xl uppercase tracking-widest">Invia ora</button>
            </form>
            <div className="mt-10 space-y-4">
               <h3 className="font-black text-slate-400 text-[10px] ml-5 uppercase tracking-widest font-sans">Storico Personale</h3>
...
function TabBtn({active, onClick, label}: any) {
  return (
    <button onClick={onClick} className={`px-5 py-3 rounded-2xl text-[9px] font-black whitespace-nowrap transition-all uppercase tracking-tighter ${active ? 'bg-blue-700 text-white shadow-lg scale-105' : 'bg-white text-slate-400'}`}>{label}</button>
  )
}
...
function CardRichiesta({r, isAdmin, onRefresh, onEdit}: any) {
  ...
  const updateStato = async (st: string) => {
    ...
    window.open(`https://wa.me/${r.todde_profiles?.telefono}?text=Ciao ${r.todde_profiles?.nome}, richiesta ${r.tipo} ${st.toUpperCase()}`)
    onRefresh()
  }
    ...
    <div className="bg-white p-6 rounded-[35px] shadow-sm border border-slate-100 mb-4 font-sans uppercase">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-6 pt-6 border-t border-slate-50">
        {isAdmin && r.stato === 'in_attesa' && (
          <><button onClick={()=>updateStato('approvato')} className="bg-green-500 text-white py-3 rounded-[20px] text-[8px] font-black uppercase">Approve</button>
            <button onClick={()=>updateStato('rifiutato')} className="bg-red-50 text-white py-3 rounded-[20px] text-[8px] font-black uppercase">Reject</button></>
        )}
