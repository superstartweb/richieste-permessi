...
export default function DashboardPage() {
  ...
  const handleAddEmployee = async (e: React.FormEvent) => {
    ...
    if (error) alert(error.message)
      ...
      const msg = `Ciao ${newEmp.nome}, benvenuto in Todde Bus. Da oggi puoi inviare le tue richieste presenze da qui: https://permessi.toddebus.it. Accedi con questa mail: ${newEmp.email}`
  ...
  const handleInvia = async (e: React.FormEvent) => {
    e.preventDefault()
    const payload = { 
      dipendente_id: userProfile.id,
      tipo: form.tipo,
      data_inizio: form.data_inizio,
      data_fine: form.tipo === 'PERMESSO' ? null : (form.data_fine || form.data_inizio),
      ora_inizio: form.tipo === 'PERMESSO' ? form.ora_inizio : null,
      ora_fine: form.tipo === 'PERMESSO' ? form.ora_fine : null,
      protocollo_malattia: form.tipo === 'MALATTIA' ? form.protocollo : null,
      stato: editId ? undefined : 'in_attesa' // Se è una modifica l'admin non cambia lo stato, se è nuova parte da in_attesa
    }
    
    let result;
    if (editId) result = await supabase.from('todde_richieste').update(payload).eq('id', editId)
    else result = await supabase.from('todde_richieste').insert([{ ...payload, stato: 'in_attesa' }])
    
    if (result.error) {
      alert("ERRORE DI SALVATAGGIO: " + result.error.message)
    } else {
      ...
      setActiveTab(userProfile.ruolo === 'admin' ? 'gestione' : 'nuova')
    }
  }

  const handleDeleteRichiesta = async (id: number) => {
    if (confirm("Sei sicuro di voler eliminare questa richiesta?")) {
      const { error } = await supabase.from('todde_richieste').delete().eq('id', id)
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Report"); XLSX.writeFile(wb, "Report_Todde.xlsx")
  }
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-10 font-sans uppercase">
          ...
          <div className="flex gap-1 overflow-x-auto no-scrollbar pb-1">
            {isAdmin && (
              <>
                ...
                <button onClick={exportExcel} className="px-5 py-3 rounded-2xl text-[9px] font-bold bg-green-500 text-white shadow-lg uppercase tracking-tighter hover:scale-105 transition-all">EXCEL</button>
              </>
            )}
            <TabBtn active={activeTab === 'nuova'} onClick={() => {setActiveTab('nuova'); setEditId(null)}} label={editId ? "MODIFICA IN CORSO" : "INVIA RICHIESTA"} />
          </div>
        </div>
      </nav>
      ...
      <main className="max-w-5xl mx-auto p-4">
        ...
        {isAdmin && activeTab === 'anagrafica' && (
          <div className="space-y-4">
            <button onClick={() => setShowAddForm(!showAddForm)} className="w-full bg-blue-700 text-white p-5 rounded-3xl font-bold shadow-lg uppercase tracking-widest">
              {showAddForm ? 'CHIUDI MODULO' : '+ AGGIUNGI DIPENDENTE'}
            </button>
...
function CardRichiesta({r, isAdmin, onRefresh, onEdit, onDelete}: any) {
  ...
  const updateStato = async (st: string) => {
    ...
    const t = `Ciao ${r.todde_profiles?.nome}, la tua richiesta di ${r.tipo} del ${r.data_inizio} è stata ${st.toUpperCase()}. https://permessi.toddebus.it`
  ...
  return (
    <div className="bg-white p-6 rounded-[35px] shadow-sm border border-slate-100 mb-4 uppercase font-sans">
      ...
      <div className="flex flex-wrap gap-2 mt-6 pt-6 border-t border-slate-50">
        {/* TASTI ADMIN: Approva / Rifiuta */}
        {isAdmin && r.stato === 'in_attesa' && (
          <>
            <button onClick={()=>updateStato('approvato')} className="flex-1 bg-green-500 text-white py-3 px-2 rounded-[20px] text-[8px] font-black uppercase shadow-md shadow-green-50">Approva</button>
            <button onClick={()=>updateStato('rifiutato')} className="flex-1 bg-red-500 text-white py-3 px-2 rounded-[20px] text-[8px] font-black uppercase shadow-md shadow-red-50">Rifiuta</button>
          </>
        )}
        
        {/* TASTO MODIFICA: Solo per Admin */}
        {isAdmin && (
          <button onClick={onEdit} className="flex-1 bg-slate-900 text-white py-3 px-2 rounded-[20px] text-[8px] font-black uppercase">Modifica</button>
        )}
        
        {/* TASTO ELIMINA: Admin sempre. Dipendente solo se in_attesa */}
        {(isAdmin || r.stato === 'in_attesa') && (
